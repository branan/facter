...
namespace facter { namespace execution {
        ...
        uint32_t timeout)
    {
        ...
        if (!IsProcessInJob(GetCurrentProcess(), nullptr, &in_job)) {
            throw execution_exception("could not determine if facter is in a object");
        }
        if (in_job) {
            ...
            if (!QueryInformationJobObject(nullptr, JobObjectBasicLimitInformation, &limits, sizeof(limits), nullptr)) {
                throw execution_exception("could not query the job object we are running under");
            }
            if (!(limits.LimitFlags & JOB_OBJECT_LIMIT_BREAKAWAY_OK)) {
        ...
        scoped_resource<HANDLE> hJob(nullptr, CloseHandle);
        if (use_job_object) {
